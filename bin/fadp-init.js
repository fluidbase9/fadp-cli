...
async function promptPem(msg) {
  log(`  ${C.cyan}?${C.reset}  ${msg}`);
  log(`  ${C.gray}  Paste your PEM (multi-line). Press ENTER on a blank line to finish.${C.reset}`);
  nl();
  const lines = [];
  return new Promise(resolve => {
    const rl = readline.createInterface({ input: process.stdin, output: process.stdout });
    rl.on("line", line => {
      if (line.trim() === "" && lines.some(l => l.startsWith("-----"))) {
        rl.close();
        resolve(lines.join("\n"));
      } else {
        lines.push(line);
...
function scaffoldSampleProject(fldpPrivateKeyPem, agentKey, walletKey) {
  ...
  const envLines = [
    ...
    fldpPrivateKeyPem ? `FLDP_PRIVATE_KEY="${fldpPrivateKeyPem.replace(/\n/g, "\\n")}"` : "# FLDP_PRIVATE_KEY=  (not generated)",
    agentKey          ? `FLUID_AGENT_KEY="${agentKey}"`                                  : "# FLUID_AGENT_KEY=  (not generated)",
    walletKey         ? `WALLET_PRIVATE_KEY="${walletKey}"`                              : "# WALLET_PRIVATE_KEY=  (not generated)",
    "",
    "FLUID_API_URL=https://fluidnative.com",
    "PORT=3001",
  ];
  fs.writeFileSync(envDest, envLines.join("\n"), { mode: 0o600 });
...
function writeEnvSnippet(fldpPrivateKeyPem, agentKey, walletKey) {
  ...
  if (fldpPrivateKeyPem) lines.push(`FLDP_PRIVATE_KEY="${fldpPrivateKeyPem.replace(/\n/g, "\\n")}"`);
  if (agentKey)          lines.push(`FLUID_AGENT_KEY="${agentKey}"`);
  if (walletKey)         lines.push(`WALLET_PRIVATE_KEY="${walletKey}"`);
  lines.push("");
...
// ─── Phase 3.5: EOA wallet key ────────────────────────────────────────────────

async function stepWalletKey(isNewAccount) {
  step("3.5", "EOA Wallet Key  (WALLET_PRIVATE_KEY)  ·  secp256k1");
  log(`  ${C.dim}Transaction Signing Key — signs USDC payments on Base mainnet.${C.reset}`);
  log(`  ${C.dim}⛓  Self-sovereign: agent holds this key locally. Never sent to any server.${C.reset}`);
  nl();

  const genWalletKey = async () => {
    const key = "0x" + crypto.randomBytes(32).toString("hex");
    log(hr());
    nl();
    warn(`${C.bold}${C.yellow}SHOWN ONCE — copy and save your WALLET_PRIVATE_KEY NOW.${C.reset}`);
    nl();
    label("WALLET_PRIVATE_KEY",   `${C.green}${key}${C.reset}`);
    nl();
    label("Transaction Sign Key",  `${C.gray}👁 shown once  💾 save to .env  🚫 never in DB${C.reset}`);
    label("Runtime wire",          `${C.gray}signed Base tx (broadcast to chain, not raw key)${C.reset}`);
    label("Next",                  `${C.gray}fund this address with USDC on Base before running agent${C.reset}`);
    nl();
    log(hr());
    nl();
    await pressEnter("I have saved my WALLET_PRIVATE_KEY — press ENTER to continue");
    return key;
  };

  const isValidKey = k =>
    typeof k === "string" && /^(0x)?[0-9a-fA-F]{64}$/.test(k.replace(/^0x/, ""));

  const normalise = k => (k.startsWith("0x") ? k : "0x" + k);

  if (isNewAccount) {
    // Scenario A — generate new or import existing
    log(`  ${C.dim}Press ENTER to generate a new wallet, or paste an existing 0x hex private key.${C.reset}`);
    nl();
    const choice = (await prompt("Existing 0x key to import, or ENTER to generate new")).trim();
    if (choice === "") return genWalletKey();
    if (isValidKey(choice)) { ok("WALLET_PRIVATE_KEY accepted (imported)"); nl(); return normalise(choice); }
    warn("Invalid key format — generating a new wallet key.");
    nl();
    return genWalletKey();

  } else {
    // Scenario B — paste existing or generate new
    log(`  ${C.dim}Paste your WALLET_PRIVATE_KEY (0x...) or press ENTER to generate a new wallet.${C.reset}`);
    nl();
    const choice = (await prompt("WALLET_PRIVATE_KEY (0x...) or ENTER to generate")).trim();
    if (choice === "") return genWalletKey();
    if (isValidKey(choice)) { ok("WALLET_PRIVATE_KEY accepted"); nl(); return normalise(choice); }
    warn("Invalid format — generating new WALLET_PRIVATE_KEY.");
    nl();
    return genWalletKey();
  }
}
